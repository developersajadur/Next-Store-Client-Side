...
export function ProfileForm({ user }: { user: TUser }) {
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
      ...
      // Handle response success or error by status or success flag
      if (response.success) {
  ...
  // const imageSrc = preview || (!imageError && user.profileImage) || "";
    ...
    <div className="space-y-6">
     ...
     <div className="relative w-fit">
        ...
        <input
          ...
          style={{ width: "96px", height: "96px" }} // match avatar size (h-24 = 96px)
        />

        {/* Label for file input wrapping the icon */}
        <label
          htmlFor="profileImageInput"
          className="absolute -bottom-1 -right-1 h-8 w-8 rounded-full bg-white flex items-center justify-center cursor-pointer shadow-md"
