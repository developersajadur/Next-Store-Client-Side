...
export function MediaLibraryDialog({
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault()
    e.stopPropagation()
    if (uploading || uploadFiles.length >= maxSelection) return
            ...
            <ScrollArea className="px-6 h-full">
              <form onSubmit={handleUpload} className="space-y-6 py-4 h-full">
                <div 
                  ...
                  onDrop={uploadFiles.length < maxSelection ? handleDrop : undefined}
                  onClick={uploadFiles.length < maxSelection ? triggerFileInput : undefined}
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground">
                      {uploadFiles.length >= maxSelection ? (
                        ...
                        `Drag and drop images here, or click to browse (${maxSelection - uploadFiles.length} remaining)`
                      )}
                    </p>
                  </div>
                  <input
                ...
                {uploadFiles.length > 0 && (
                  <div className="space-y-6 pb-4">
                    <div className="flex justify-between items-center">
                      <h3 className="font-medium text-lg">Files to Upload ({uploadFiles.length}/{maxSelection})</h3>
                      <Button 
                        variant="ghost" 
